...
uint8_t button1, button2, button1_ADC, button2_ADC, poweroffbtn, speed_btn1,speed_test;
float speed_btn2;
...
#ifdef CONTROL_PPM
    //Для нашего пульта идеально но не выключается(не выключался изза калибровки пульта.)
    cmd1 = CLAMP((ppm_captured_value[3] - 500) * 2, -1000, 1000);
    cmd2 = CLAMP((ppm_captured_value[2] - 500) * 2, -1000, 1000);

    // button1 = ppm_captured_value[5] > 100;
    // float scale = ppm_captured_value[4] / 1000.0f;
    poweroffbtn = ppm_captured_value[9];
    //правая крутилка
    speed_btn1 = ppm_captured_value[7];
    //второй слева рычаг
    speed_btn2 = ppm_captured_value[8];
...
#else
    if (speed_btn2 == 232) {
      //правая крутилка для управления скоростью
      float speed_test=((((int)speed_btn1/2.32)*0.02)*100)/100;
      speedR = CLAMP(speed * speed_test - steer * STEER_COEFFICIENT, -1000, 1000);
      speedL = CLAMP(speed * speed_test + steer * STEER_COEFFICIENT, -1000, 1000);
    }
    else
    {//slow always
      buzzerFreq = 0;
...
// ####### DEBUG SERIAL OUT #######
      ...
      //speed_up_button
      setScopeChannel(8, (int)speed_btn1);
      setScopeChannel(9, (int)speed_test);
    ...
    if (HAL_GPIO_ReadPin(BUTTON_PORT, BUTTON_PIN) && weakr == 0 && weakl == 0)
    {
      ...
      { // wait, till the power button is released. Otherwise cutting power does nothing
#ifdef SOFTWATCHDOG_TIMEOUT
        __HAL_TIM_SET_COUNTER(&htim3, 0); // Kick the Watchdog
